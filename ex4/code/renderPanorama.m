...
% were backwarped each from the relevant frame imfig using homography Hfig.
    ...
    for i = 1:length(im)
       t = applyHomography([0, 0 ; 0, size(im{i},1) ;...
           size(im{i},2), 0 ; size(im{i},2),  size(im{i},1) ], H{i});
       mn = round(min(t));
...
%%
    ...
    for i = 1:length(im)
       ...
       if i == 1
            ...
            try
                % try loading previosly calculated mask for acceleration
                load M.mat 
                mask = M{i-1};
                s = 0;
            catch
                ...
                roi(mutal==2) = abs(panorama(mutal==2) -...
                    c_im(mutal==2)).^2; %intersection of images
                roi = roi(:,max(extr_x(1,i) - 1,1): min(extr_x(2,i-1) + 1,...
                    size(roi,2)) );%taking only the true roi

                mask(:,max(extr_x(1,i) - 1,1): min(extr_x(2,i-1) +...
                    1,size(panorama,2))) = dynamic_seam(roi);
                mask(:,1:extr_x(1,i) - 1) = 1;
                mask(:,extr_x(2,i-1) + 1 : end) = 0;

                M{i-1} = mask;
                %%uncomment for basic mask
                %mask(:,1:strip(i)) = 1;
                
            end
            panorama = pyramidBlending(panorama, c_im, mask, 6,5,4);
       end
